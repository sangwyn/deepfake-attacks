...
Для ассистента, который **эксплуатирует** систему через SSH: обновляет,
запускает, следит, читает результаты. Читать целиком до первой команды.

Отладкой кода занимается другой агент по `docs/MAINTAINER_HANDOFF_RU.md`.
Встретив симптом, которого нет в таблице раздела 8, — остановитесь и передайте
ему, не обходя проверку.
