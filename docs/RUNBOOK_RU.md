...
перепроверяется под блокировкой карты непосредственно перед запуском. Это
защищает и нас, и соседа. Не поднимать `--max-running` выше 1 и не уменьшать
`--headroom-mb` на общей машине.
