...
Доступные атаки: `fgsm`, `ifgsm`, `pgd`, `mifgsm`, `di-mi-fgsm`,
`ti-di-mi-fgsm`, `frequency-eot`, `mig-cow`, `dd-fcma`, `prototype`.
Реализована пока только `ifgsm` — остальные агент пишет сам, это его работа.
