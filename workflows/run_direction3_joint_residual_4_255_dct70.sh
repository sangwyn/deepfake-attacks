...
LOG="$ROOT/logs/direction3_joint_residual_4_255_dct70.log"
CONFIG="$ROOT/configs/direction3_joint_residual_4_255_dct70_heldout.yaml"
mkdir -p "$ROOT/logs" "$ROOT/experiments/direction3"
exec >> "$LOG" 2>&1
printf '[D3-JOINT-RESIDUAL-4/255-DCT70] started %s\n' "$(date -Is)"
GPU=""
while [[ -z "$GPU" ]]; do
  ...
  if [[ -z "$GPU" ]]; then printf '[D3-JOINT-RESIDUAL-4/255-DCT70] waiting for GPU\n'; sleep 60; fi
done
printf '[D3-JOINT-RESIDUAL-4/255-DCT70] selected GPU %s\n' "$GPU"
