...
LOG="$ROOT/logs/direction3_lowres32.log"
CONFIG="$ROOT/configs/direction3_universal_lowres32.yaml"
mkdir -p "$ROOT/logs" "$ROOT/experiments/direction3"
exec >> "$LOG" 2>&1

printf '[D3-LOWRES] started %s\n' "$(date -Is)"
choose_gpu() {
...
while ! GPU="$(choose_gpu)"; do printf '[D3-LOWRES] waiting for GPU\n'; sleep 60; done
printf '[D3-LOWRES] selected GPU %s at %s\n' "$GPU" "$(date -Is)"
CUDA_VISIBLE_DEVICES="$GPU" "$ROOT/.venv/bin/python" "$ROOT/workflows/direction3_universal.py" --config "$CONFIG"
status=$?
printf '[D3-LOWRES] finished status=%s at %s\n' "$status" "$(date -Is)"
exit "$status"
