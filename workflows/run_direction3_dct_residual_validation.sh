...
LOG="$ROOT/logs/direction3_dct_residual_heldout.log"
CONFIG="$ROOT/configs/direction3_dct_residual_validation.yaml"
mkdir -p "$ROOT/logs" "$ROOT/experiments/direction3"
exec >> "$LOG" 2>&1
printf '[D3-DCT-RESIDUAL] started %s\n' "$(date -Is)"
GPU=""
while [[ -z "$GPU" ]]; do
  ...
  if [[ -z "$GPU" ]]; then printf '[D3-DCT-RESIDUAL] waiting for GPU\n'; sleep 60; fi
done
printf '[D3-DCT-RESIDUAL] selected GPU %s\n' "$GPU"
