...
LOG="$ROOT/logs/direction3_joint_residual_4_255_vit70_heldout.log"
CONFIG="$ROOT/configs/direction3_joint_residual_4_255_vit70_validation.yaml"
mkdir -p "$ROOT/logs" "$ROOT/experiments/direction3"
exec >> "$LOG" 2>&1
printf '[D3-JOINT-RESIDUAL-4/255-VIT70] started %s\n' "$(date -Is)"
GPU=""
while [[ -z "$GPU" ]]; do
  ...
  if [[ -z "$GPU" ]]; then printf '[D3-JOINT-RESIDUAL-4/255-VIT70] waiting for GPU\n'; sleep 60; fi
done
printf '[D3-JOINT-RESIDUAL-4/255-VIT70] selected GPU %s\n' "$GPU"
