...
LOG="$ROOT/logs/direction3_ablation_heldout.log"
CONFIG="$ROOT/configs/direction3_ablation_heldout.yaml"
mkdir -p "$ROOT/logs" "$ROOT/experiments/direction3"
exec >> "$LOG" 2>&1
printf '[D3-ABLATION] started %s\n' "$(date -Is)"
GPU=""
while [[ -z "$GPU" ]]; do
  ...
  if [[ -z "$GPU" ]]; then printf '[D3-ABLATION] waiting for GPU\n'; sleep 60; fi
done
printf '[D3-ABLATION] selected GPU %s\n' "$GPU"
