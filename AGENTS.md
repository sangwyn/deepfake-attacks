...
8. Do not install dependencies, download checkpoints, train models, commit, push, access the network, or touch another worktree from an automated agent session. A worker may stage the inputs it froze with `git add` under `attacks/`, `configs/experiments/`, `tests/`, and `tracking/jobs/` — the runner rejects an input that is not in Git, and staging records that intent without writing project history. Staging anything else, or committing, remains forbidden.
...
10. `smoke` permits focused tests and a queued experiment for at most 16 images. `development` uses the frozen development manifest `manifests/celebA/test_fake.jsonl`, whose hash is recorded in `manifests/celebA/catalog.json`; use it whole and never construct a substitute. `full` requires explicit user authorization and frozen finalists; never infer authorization from an earlier scope.
