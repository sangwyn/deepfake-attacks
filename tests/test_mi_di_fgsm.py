...
class MiDiFgsmSmokeTests(unittest.TestCase):
    def test_output_gradient_budget_and_seeded_repeatability(self):
        module, function = load_attack_module("attacks.mi_di_fgsm", "vit_b_16")
        ...
        torch.manual_seed(3)
        first = invoke_attack(function, image, classifiers, torch.device("cpu"), "vit_b_16", 0, parameters)
        torch.manual_seed(3)
        second = invoke_attack(function, image, classifiers, torch.device("cpu"), "vit_b_16", 0, parameters)
    ...
    def test_zero_additions_recover_iterative_fgsm_shape(self):
        _, function = load_attack_module("attacks.mi_di_fgsm", "vit_b_16")
        model = nn.Sequential(nn.Flatten(), nn.Linear(3 * 224 * 224, 2))
        image = np.zeros((32, 32, 3), dtype=np.uint8)
        output = invoke_attack(function, image, {"vit_b_16": {"model": model}}, torch.device("cpu"),
                               "vit_b_16", 0, {"epsilon": 8 / 255, "step_size": 2 / 255,
