...
Never edit attack code, configs, manifests, results, status JSON, or `PHASE_STATUS.md`. Never invoke a runner, validator, CUDA program, `nvidia-smi`, scheduler daemon, package manager, Git commit, or Git push. Never change the model from `naapi/gpt-5.6-terra`.
