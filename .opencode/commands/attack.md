...
3. Run permitted CPU/unit checks with the project interpreter, never a bare `python3`: the dependencies live in `.venv` and a system interpreter cannot import numpy or torch. `pytest` is deliberately absent from the frozen environment, so run a test file directly:

   ```bash
   PYTHONPATH=. .venv/bin/python tests/<test file>.py
   ```

   Never run the experiment or verifier directly.
...
Write your status to `STATUS_FILE` exactly as given in `$3`. Do not rename it, do not choose a different path, and never write `status.worker.json` yourself: that file is produced by the controller, which copies your document there before replacing `STATUS_FILE` with its own. A status written anywhere but `$3` does not exist as far as the controller is concerned, and the task fails with the GPU work already done.

Only the deterministic controller may reconcile the status to `running`, `passed`, `failed`, or `cancelled`. It polls the queue after you exit and reads the verifier report. A final `passed` status additionally requires non-empty existing `configs`, `results`, and `evidence`, plus a valid `verifier_report`. Never claim `passed` yourself: the controller rejects a worker status carrying a controller-owned outcome, which fails the task. Never update `PHASE_STATUS.md` from a queued result.
